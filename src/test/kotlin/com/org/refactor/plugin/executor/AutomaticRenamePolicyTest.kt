...
class AutomaticRenamePolicyTest : BasePlatformTestCase() {
    ...
    fun `acceptAll selects every automatic rename suggestion`() {
        ...
        assertEquals(suggestions, renamer.selected)
    }

    private class RecordingAutomaticRenamer(
        private val suggestions: Map<PsiNamedElement, String>,
    ) : AutomaticRenamer() {

        val selected = linkedMapOf<PsiNamedElement, String>()

        init {
            myElements.addAll(suggestions.keys)
        }

        override fun getNewName(element: PsiNamedElement): String? = suggestions[element]
